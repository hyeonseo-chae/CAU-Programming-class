...
bool AUX_RGBImageRec::loadFile(const char* Filename)
{
    ...
    wchar_t pwFilename[1024];
    CharToWChar(Filename, pwFilename);
    ...
    HANDLE DIBHandle = LoadImage(0, pwFilename, IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION | LR_LOADFROMFILE);  // use windows to get header info of bitmap - assume no errors in header format
